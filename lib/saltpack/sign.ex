defmodule Saltpack.Sign do
  ...
  def create_message(payload, private, mode, nil),
    do: create_message(payload, private, mode, Kcl.derive_public_key(private, :sign))
  ...
  defp pack_payload(
    ...
    pack_payload(rest, header_hash, private, public, nonce_count + 1, [
      code_payload(m, header_hash, private, public, nonce(nonce_count)) | acc
    ])
  end
  ...
  defp verify_msg_match({signature, hhash, opub}, text) do
    if Kcl.valid_signature?(
         signature |> Msgpax.unpack!(),
         mode_constant(2) <> :crypto.hash(:sha512, hhash <> text),
         opub
       ),
       do: opub,
       else: sign_error()
  ...
  defp payloads_open({payloads, hhash, pub}, nonce_count, acc) do
    ...
    if not Kcl.valid_signature?(
         tsig,
         mode_constant(1) <> :crypto.hash(:sha512, hhash <> nonce <> data),
         pub
       ),
       do: sign_error()
