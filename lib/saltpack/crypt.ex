defmodule Saltpack.Crypt do
  ...
  defp pack_header(recipients, our_private, our_public) do
    ...
    hbody = IO.iodata_to_binary(Msgpax.pack!(
            [@format, @version, @mode,
             Msgpax.Bin.new(epub),  # ephemeral public key
    ...
    {hbody |> Msgpax.Bin.new |> Msgpax.pack! |> IO.iodata_to_binary,
     recipients |> recipient_boxes(first_bytes(hhash, 24), @nil_key, our_private, []) |> Enum.map(fn {_k, v} -> v |> last_bytes(32) end),
     payload_key, hhash
    }
  end
  ...
  defp authenticators([r|rest], hash, acc), do: authenticators(rest, hash, [Msgpax.Bin.new(:crypto.hmac(:sha512, r, hash, 32))|acc])
  ...
  defp header_verify(packet, priv)  do
    ...
    {rest, index, payload_key, hhash,
     @nil_key
      |> stateless_box(first_bytes(hhash, 24), priv, Kcl.secretunbox(secret_box, @header_sbox_nonce, payload_key))
      |> last_bytes(32)}
  end
  ...
  defp payload_info(recipients, priv, tpub) do
    ...
    {index, recipients |> Enum.at(index) |> Enum.at(1) |> stateless_unbox(@header_box_nonce, opriv, tpub)}
  end

  defp stateless_unbox(message, nonce, private, public), do: message |> Kcl.unbox(nonce, private, public) |> drop_state
  defp stateless_box(message, nonce, private, public), do: message |> Kcl.box(nonce, private, public) |> drop_state
